...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.pendencium.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.pendencia.update", [$pendencium->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('idestabelecimento') ? 'is-invalid' : '' }}" name="idestabelecimento_id" id="idestabelecimento_id" required>
                    @foreach($idestabelecimentos as $id => $idestabelecimento)
                        <option value="{{ $id }}" {{ (old('idestabelecimento_id') ? old('idestabelecimento_id') : $pendencium->idestabelecimento->id ?? '') == $id ? 'selected' : '' }}>{{ $idestabelecimento }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('pendencia') ? 'is-invalid' : '' }}" name="pendencia" id="pendencia">{!! old('pendencia', $pendencium->pendencia) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('prazo') ? 'is-invalid' : '' }}" type="text" name="prazo" id="prazo" value="{{ old('prazo', $pendencium->prazo) }}" required>
                @if($errors->has('prazo'))
                    <div class="invalid-feedback">
                        {{ $errors->first('prazo') }}
                    </div>
