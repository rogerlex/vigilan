...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.baixaduam.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.baixaduams.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('processos') ? 'is-invalid' : '' }}" name="processos[]" id="processos" multiple required>
                    @foreach($processos as $id => $processo)
                        <option value="{{ $id }}" {{ in_array($id, old('processos', [])) ? 'selected' : '' }}>{{ $processo }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('id_duam') ? 'is-invalid' : '' }}" type="text" name="id_duam" id="id_duam" value="{{ old('id_duam', '') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('data_emissao') ? 'is-invalid' : '' }}" type="text" name="data_emissao" id="data_emissao" value="{{ old('data_emissao') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('data_pagamento') ? 'is-invalid' : '' }}" type="text" name="data_pagamento" id="data_pagamento" value="{{ old('data_pagamento') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('valorpago') ? 'is-invalid' : '' }}" type="number" name="valorpago" id="valorpago" value="{{ old('valorpago', '') }}" step="0.01" required>
