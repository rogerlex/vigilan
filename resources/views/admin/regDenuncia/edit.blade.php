...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.regDenuncium.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.reg-denuncia.update", [$regDenuncium->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="data_denuncia">{{ trans('cruds.regDenuncium.fields.data_denuncia') }}</label>
                <input class="form-control date {{ $errors->has('data_denuncia') ? 'is-invalid' : '' }}" type="text" name="data_denuncia" id="data_denuncia" value="{{ old('data_denuncia', $regDenuncium->data_denuncia) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('denunciante') ? 'is-invalid' : '' }}" type="text" name="denunciante" id="denunciante" value="{{ old('denunciante', $regDenuncium->denunciante) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('contato') ? 'is-invalid' : '' }}" type="text" name="contato" id="contato" value="{{ old('contato', $regDenuncium->contato) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('descricao') ? 'is-invalid' : '' }}" name="descricao" id="descricao">{!! old('descricao', $regDenuncium->descricao) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('origem') ? 'is-invalid' : '' }}" name="origem_id" id="origem_id" required>
                    @foreach($origems as $id => $origem)
                        <option value="{{ $id }}" {{ (old('origem_id') ? old('origem_id') : $regDenuncium->origem->id ?? '') == $id ? 'selected' : '' }}>{{ $origem }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('categoria') ? 'is-invalid' : '' }}" name="categoria_id" id="categoria_id" required>
                    @foreach($categorias as $id => $categoria)
                        <option value="{{ $id }}" {{ (old('categoria_id') ? old('categoria_id') : $regDenuncium->categoria->id ?? '') == $id ? 'selected' : '' }}>{{ $categoria }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status_id" id="status_id" required>
                    @foreach($statuses as $id => $status)
                        <option value="{{ $id }}" {{ (old('status_id') ? old('status_id') : $regDenuncium->status->id ?? '') == $id ? 'selected' : '' }}>{{ $status }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('feedback') ? 'is-invalid' : '' }}" name="feedback" id="feedback">{!! old('feedback', $regDenuncium->feedback) !!}</textarea>
