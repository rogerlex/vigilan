...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.denuncia.update", [$denuncium->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="data_denuncia">{{ trans('cruds.denuncium.fields.data_denuncia') }}</label>
                <input class="form-control date {{ $errors->has('data_denuncia') ? 'is-invalid' : '' }}" type="text" name="data_denuncia" id="data_denuncia" value="{{ old('data_denuncia', $denuncium->data_denuncia) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('denunciante') ? 'is-invalid' : '' }}" type="text" name="denunciante" id="denunciante" value="{{ old('denunciante', $denuncium->denunciante) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('contato_denunciante') ? 'is-invalid' : '' }}" type="text" name="contato_denunciante" id="contato_denunciante" value="{{ old('contato_denunciante', $denuncium->contato_denunciante) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{!! old('description', $denuncium->description) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('categories') ? 'is-invalid' : '' }}" name="categories[]" id="categories" multiple required>
                    @foreach($categories as $id => $category)
                        <option value="{{ $id }}" {{ (in_array($id, old('categories', [])) || $denuncium->categories->contains($id)) ? 'selected' : '' }}>{{ $category }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('tags') ? 'is-invalid' : '' }}" name="tags[]" id="tags" multiple required>
                    @foreach($tags as $id => $tag)
                        <option value="{{ $id }}" {{ (in_array($id, old('tags', [])) || $denuncium->tags->contains($id)) ? 'selected' : '' }}>{{ $tag }}</option>
                    @endforeach
                </select>
                @if($errors->has('tags'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('tratativa') ? 'is-invalid' : '' }}" name="tratativa" id="tratativa">{!! old('tratativa', $denuncium->tratativa) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('data_conclusao') ? 'is-invalid' : '' }}" type="text" name="data_conclusao" id="data_conclusao" value="{{ old('data_conclusao', $denuncium->data_conclusao) }}">
