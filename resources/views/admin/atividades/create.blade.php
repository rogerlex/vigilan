...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.atividade.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.atividades.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('numeroprocesso') ? 'is-invalid' : '' }}" name="numeroprocesso_id" id="numeroprocesso_id" required>
                    @foreach($numeroprocessos as $id => $numeroprocesso)
                        <option value="{{ $id }}" {{ old('numeroprocesso_id') == $id ? 'selected' : '' }}>{{ $numeroprocesso }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('vista_num_processo') ? 'is-invalid' : '' }}" type="text" name="vista_num_processo" id="vista_num_processo" value="{{ old('vista_num_processo', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('tipo_de_processo') ? 'is-invalid' : '' }}" name="tipo_de_processo_id" id="tipo_de_processo_id" required>
                    @foreach($tipo_de_processos as $id => $tipo_de_processo)
                        <option value="{{ $id }}" {{ old('tipo_de_processo_id') == $id ? 'selected' : '' }}>{{ $tipo_de_processo }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('relatorio_da_atividade') ? 'is-invalid' : '' }}" name="relatorio_da_atividade" id="relatorio_da_atividade">{!! old('relatorio_da_atividade') !!}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('equipe_responsavels') ? 'is-invalid' : '' }}" name="equipe_responsavels[]" id="equipe_responsavels" multiple required>
                    @foreach($equipe_responsavels as $id => $equipe_responsavel)
                        <option value="{{ $id }}" {{ in_array($id, old('equipe_responsavels', [])) ? 'selected' : '' }}>{{ $equipe_responsavel }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('data_atividade') ? 'is-invalid' : '' }}" type="text" name="data_atividade" id="data_atividade" value="{{ old('data_atividade') }}" required>
