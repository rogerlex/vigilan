...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.tipoestabelecimentos.update", [$tipoestabelecimento->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="tipoestabelecimento">{{ trans('cruds.tipoestabelecimento.fields.tipoestabelecimento') }}</label>
                <input class="form-control {{ $errors->has('tipoestabelecimento') ? 'is-invalid' : '' }}" type="text" name="tipoestabelecimento" id="tipoestabelecimento" value="{{ old('tipoestabelecimento', $tipoestabelecimento->tipoestabelecimento) }}" required>
