...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.visitum.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.visita.update", [$visitum->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('motivo') ? 'is-invalid' : '' }}" name="motivo" id="motivo" required>
                    ...
                    @foreach(App\Models\Visitum::MOTIVO_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('motivo', $visitum->motivo) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('estabelecimentos') ? 'is-invalid' : '' }}" name="estabelecimentos[]" id="estabelecimentos" multiple required>
                    @foreach($estabelecimentos as $id => $estabelecimento)
                        <option value="{{ $id }}" {{ (in_array($id, old('estabelecimentos', [])) || $visitum->estabelecimentos->contains($id)) ? 'selected' : '' }}>{{ $estabelecimento }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('dataagendamento') ? 'is-invalid' : '' }}" type="text" name="dataagendamento" id="dataagendamento" value="{{ old('dataagendamento', $visitum->dataagendamento) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('equipes') ? 'is-invalid' : '' }}" name="equipes[]" id="equipes" multiple required>
                    @foreach($equipes as $id => $equipe)
                        <option value="{{ $id }}" {{ (in_array($id, old('equipes', [])) || $visitum->equipes->contains($id)) ? 'selected' : '' }}>{{ $equipe }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('relatorio_observado') ? 'is-invalid' : '' }}" name="relatorio_observado" id="relatorio_observado">{!! old('relatorio_observado', $visitum->relatorio_observado) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('status_visita') ? 'is-invalid' : '' }}" name="status_visita_id" id="status_visita_id" required>
                    @foreach($status_visitas as $id => $status_visita)
                        <option value="{{ $id }}" {{ (old('status_visita_id') ? old('status_visita_id') : $visitum->status_visita->id ?? '') == $id ? 'selected' : '' }}>{{ $status_visita }}</option>
                    @endforeach
                </select>
